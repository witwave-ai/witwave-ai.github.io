...
whether the system can complete a task. The distinction is the control relationship between the human, the runtime, and
the agent.
...
That question keeps the definition practical. It also avoids two common mistakes.
...
but the absence of any layer weakens the autonomy claim.
...
Activation is the difference between "I can ask it to do something" and "it can notice when work may need to happen."
...
If the workflow simply follows a fixed script, it is automation with model assistance. It becomes more autonomous if it
can decide what changed, identify anomalies, ask follow-up questions, select sources, delay publication when confidence
is low, and record why it made those choices.
...
It also requires organizational maturity. Someone has to decide what the agent is allowed to decide.
...
The next useful boundary in agentic systems is not "agent versus non-agent." That boundary has become too blurry. The
...
interface matters most. If an agent is autonomous, the operating model matters most.
...
support pieces of the framework, but the commanded-versus-governed distinction is the paper's own synthesis.
